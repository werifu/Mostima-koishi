...
export function apply(ctx: Context) {
    ...
    //     getRandomArkMusic().then((music: Music)=>{
    //         let data: string = `<?xml version='1.0' encoding='UTF-8' standalone='yes' ?><msg serviceID="2" templateID="1" action="web" brief="&#91;分享&#93; ${music.name}" sourceMsgId="0" url="${music.music_url}" flag="0" adverSign="0" multiMsgFlag="0" ><item layout="2"><audio cover="${music.img_src}" src="https://music.163.com/song/media/outer/url?id=409650368.mp3" /><title>${music.name}</title><summary>${music.artist}</summary></item><source name="网易云音乐" icon="https://pic.rmb.bdstatic.com/911423bee2bef937975b29b265d737b3.png" url="http://web.p.qq.com/qqmpmobile/aio/app.html?id=1101079856" action="app" a_actionData="com.netease.cloudmusic" i_actionData="tencent100495085://" appid="100495085" /></msg>`;
    //         ctx.bots[0].sendPrivateMsg(owner, CQCode.stringify('xml',{data: data}));
    //     })
    // })
    ...
    ctx.middleware(async (meta, next) => {
        ...
        if (msg.includes('!音角') || msg.includes('！音角')) {
            ...
            let tag = msg.match(search_exp)[0].substring(4);
            if (tag === '') {
...
function getRandomArkMusic(): Promise<Music> {
    let music = {id: 0, name: "", artist: "", img_src: ""};
    let limit: number = 200;
    
    return getRandomArkAlbum().then((album_id: number)=>{
        ...
        return axios.get(album_songs_api).then((res:any)=> {
            ...
            return music;
        }).catch((err) => {console.log(err); return music;})
    });
}
